        ...
        {list.map((d) => (
          <Link key={d.id} to={`/passport/${g.id}/${d.id}`} className="card card-hover group flex flex-col p-5">
            ...
            <div className="mt-4 grid grid-cols-4 gap-2">
              <Mini value={String(d.ntr)} label={t('pp.kpi.ntr')} tone={d.ntr > 0 ? 'red' : 'green'} />
              <Mini value={d.workers.toLocaleString('ru-RU')} label={t('pp.kpi.workers')} tone="blue" />
              <Mini value={d.ltir.toFixed(3)} label="LTIR" tone={d.ltir > 0 ? 'amber' : 'green'} />
              <Mini value={String(d.nearMiss)} label="Near Miss" tone="green" />
            </div>
